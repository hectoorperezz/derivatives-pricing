...
def test_explicit_stability_threshold():
    ...
    _, stable = hapi.solve_heat_equation(
        alternating_mode,
        kappa=kappa,
        M=M,
        T=stable_T,
    ...
    _, unstable = hapi.solve_heat_equation(
        alternating_mode,
        kappa=kappa,
        M=M,
        T=unstable_T,
...
def test_explicit_convergence():
    ...
    coarse_x, coarse = hapi.solve_heat_equation(
        lambda x: np.sin(math.pi * x / M),
        kappa=kappa,
        M=M,
        T=T,
    ...
    fine_x, fine = hapi.solve_heat_equation(
        lambda x: np.sin(math.pi * x / M),
        kappa=kappa,
        M=M,
        T=T,
...
def test_black_scholes_heat_matches_closed_form(call, params):
    closed_form = hapi.get_price_bs_european(**params, call=call)
    heat = hapi.get_price_bs_european_heat(**params, call=call)
...
def test_heat_solver_validation(kwargs, message):
    ...
    with pytest.raises(ValueError, match=message):
        hapi.solve_heat_equation(**params)
...
def test_black_scholes_heat_validation(kwargs, message):
    ...
    with pytest.raises(ValueError, match=message):
        hapi.get_price_bs_european_heat(**params)
