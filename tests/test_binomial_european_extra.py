"""Additional tests for binomial pricing of European options.

Tests cover:
- All intermediate tree nodes from a T=4 put scenario.
...
class TestTreeNodes:
    ...
    def test_t3_node_195(self):
        price = pricing.get_price_binomial_european(195.3125, K, 1, R, u, d, call=False)
        np.testing.assert_allclose(price, 0.0, atol=1e-4)

    def test_t3_node_132(self):
        price = pricing.get_price_binomial_european(132.8125, K, 1, R, u, d, call=False)
        np.testing.assert_allclose(price, 0.0, atol=1e-4)

    def test_t3_node_90(self):
        price = pricing.get_price_binomial_european(90.3125, K, 1, R, u, d, call=False)
        np.testing.assert_allclose(price, 15.8259, atol=1e-3)

    def test_t3_node_61(self):
        price = pricing.get_price_binomial_european(61.4125, K, 1, R, u, d, call=False)
        np.testing.assert_allclose(price, 43.3494, atol=1e-3)

    # t=2 nodes
    def test_t2_node_156(self):
        price = pricing.get_price_binomial_european(156.25, K, 2, R, u, d, call=False)
        np.testing.assert_allclose(price, 0.0, atol=1e-4)

    def test_t2_node_106(self):
        price = pricing.get_price_binomial_european(106.25, K, 2, R, u, d, call=False)
        np.testing.assert_allclose(price, 7.5361, atol=1e-3)

    def test_t2_node_72(self):
        price = pricing.get_price_binomial_european(72.25, K, 2, R, u, d, call=False)
        np.testing.assert_allclose(price, 28.1787, atol=1e-3)

    # t=1 nodes
    def test_t1_node_125(self):
        price = pricing.get_price_binomial_european(125.0, K, 3, R, u, d, call=False)
        np.testing.assert_allclose(price, 3.5886, atol=1e-3)

    def test_t1_node_85(self):
        price = pricing.get_price_binomial_european(85.0, K, 3, R, u, d, call=False)
        np.testing.assert_allclose(price, 17.0071, atol=1e-3)

    # t=0
    def test_t0_root(self):
        price = pricing.get_price_binomial_european(S0, K, T, R, u, d, call=False)
...
class TestPutCallParity:
    ...
    def test_parity(self, St, K_, T_):
        call = pricing.get_price_binomial_european(St, K_, T_, R, u, d, call=True)
        put  = pricing.get_price_binomial_european(St, K_, T_, R, u, d, call=False)
        lhs = call - put
        rhs = St - K_ / (1 + R) ** T_
...
class TestEdgeCases:
    ...
    def test_T1_put_in_the_money(self):
        ...
        price = pricing.get_price_binomial_european(St, K_, T_, R, u, d, call=False)
    ...
    def test_T1_call_out_of_the_money(self):
        ...
        price = pricing.get_price_binomial_european(St, K_, T_, R, u, d, call=True)
        np.testing.assert_allclose(price, expected, atol=1e-10)

    def test_call_and_put_nonnegative(self):
        """Option prices must always be >= 0."""
        for call in [True, False]:
            price = pricing.get_price_binomial_european(S0, K, T, R, u, d, call=call)
    ...
    def test_put_deep_itm_upper_bound(self):
        ...
        price = pricing.get_price_binomial_european(1.0, K, T, R, u, d, call=False)
