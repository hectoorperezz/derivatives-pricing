...
def test_dividend_recovery(call):
    ...
    plain = pricing.get_price_bs_european(**params)
    dividend = pricing.get_price_bs_european_dividend(**params, q=0.0)
...
def test_dividend_reference(call, expected):
    ...
    value = pricing.get_price_bs_european_dividend(
        **DIVIDEND_PARAMS,
        call=call,
    )
...
def test_dividend_parity():
    ...
    call = pricing.get_price_bs_european_dividend(**params, call=True)
    put = pricing.get_price_bs_european_dividend(**params, call=False)
...
def test_dividend_reduction(call):
    ...
    dividend = pricing.get_price_bs_european_dividend(**params)
    plain = pricing.get_price_bs_european(
        St=adjusted_spot,
        K=params["K"],
        T=params["T"],
        r=params["r"],
...
def test_fx_symmetry(call):
    ...
    fx = pricing.get_price_fx_option(**params, call=call)
    dividend = pricing.get_price_bs_european_dividend(
        St=params["St"],
        K=params["K"],
        T=params["T"],
        r=params["r_d"],
...
def test_fx_zero_foreign_rate():
    ...
    fx = pricing.get_price_fx_option(**params, r_f=0.0)
    plain = pricing.get_price_bs_european(
        St=params["St"],
        K=params["K"],
        T=params["T"],
...
def test_future_reference(call, expected):
    ...
    # QuantLib is not a project dependency.
    value = pricing.get_price_future_option(
        **FUTURE_PARAMS,
        call=call,
    )
...
def test_future_symmetry(call):
    ...
    future = pricing.get_price_future_option(**params, call=call)
    dividend = pricing.get_price_bs_european_dividend(
        St=params["F0"],
        K=params["K"],
        T=params["T"],
...
def test_future_parity():
    ...
    call = pricing.get_price_future_option(**params, call=True)
    put = pricing.get_price_future_option(**params, call=False)
...
def test_future_zero_rate():
    ...
    future = pricing.get_price_future_option(**params)
    plain = pricing.get_price_bs_european(
        St=params["F0"],
        K=params["K"],
        T=params["T"],
...
def test_cost_of_carry_validation(pricing_function, kwargs, message):
    ...
    if pricing_function is pricing.get_price_future_option:
